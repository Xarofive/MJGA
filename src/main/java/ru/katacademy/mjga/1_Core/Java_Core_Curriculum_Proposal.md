...
Документ актуализирован 12 июня 2026 года и проектирует Java Core как прямое продолжение завершенного Java Base на основе файла `../0_Base/java_base_student_portrait.md`.

Статус реализации:

- модуль `1.0_Переход_в_Core_типы_пакеты_и_рабочий_процесс` уже подготовлен: 4 содержательные лекции, отдельный материал для устного ревью и 12 практических задач;
- модуль `1.1_Инкапсуляция_и_конструкторы` содержит 4 короткие обязательные лекции; практика и мини-проект пока не подготовлены;
- раздел `Справочник` содержит необязательные карты и таблицы для быстрого поиска и повторения; сейчас подготовлена карта основных ключевых слов Java;
- разделы ниже для модулей `1.2-1.10` остаются целевой программой, а не описанием уже готового контента.
...
- студент умеет использовать enhanced `for`, составные присваивания и два базовых преобразования: `int` -> `double` и `(int) double`; полная система numeric promotion, диапазоны, переполнение и остальные числовые преобразования не изучены;
- студент умеет вызывать готовые конструкторы через `new`, но не умеет объявлять собственные конструкторы, задавать их параметры и перегружать их;
- студент не изучал `byte`, `short`, `long`, `float`, Maven/Gradle, JUnit, инкапсуляцию, наследование, интерфейсы, `enum`, полноценный Collections Framework, generics как тему, Files API, Stream API, lambdas, SQL/JDBC и многопоточность;
- Base уже дал прикладные `ArrayList`/`HashMap`, но не дал `List`/`Map`/`Set` как интерфейсы;
- Base уже дал простые классы и объекты, но не дал коммерчески нормальную объектную модель с `private`, объявлением собственных конструкторов, инвариантами, `equals/hashCode` и пакетами.
...
Методический принцип объяснения: обязательный маршрут дает минимальный рабочий навык через проблему, маленький код, ошибку/антипример и ближайшую практику. Одна лекция концентрируется на одной группе тесно связанных действий и не пытается сразу раскрыть все инженерные следствия темы. Новый термин при первом содержательном появлении выделяется жирным и кратко объясняется в тексте, а в конце лекции повторяется в разделе `Словарь`. Аналогии используются только там, где они действительно сокращают путь к пониманию. Продвинутые детали выносятся в углубление или возвращаются позже, когда они нужны в коде.
...
**Новые знания:** `byte`, `short`, `long`, `float`, диапазоны, переполнение, полная система numeric promotion и cast, суффиксы `L`/`f`, `package`, структура исходников.

**Почему здесь:** Base показал только преобразования `int` -> `double` и `(int) double`, но не дал полной системы примитивов и числовых преобразований. Без нее нельзя честно начинать Core: `long` нужен для идентификаторов и времени, а `float` и `double` нужно различать.
...
**Почему здесь:** Base уже дал классы, состояние и вызов готовых конструкторов через `new`, но не дал `private`, объявление собственных constructors и getters/setters. Эти темы нужны до наследования, коллекций объектов и тестирования доменной логики.
...
|---|---|---|---|
| 1. Повтор Base без повторения Base | Диагностика и рефакторинг: методы, условия, циклы, массивы, строки, `ArrayList`/`HashMap`, крайние случаи, ответственность метода, кодстайл | Весь Base | Рефакторинг расчета скидки; подсчет слов длиннее порога |
...
| 3. Числовые преобразования и переполнение | Полная картина cast и numeric promotion, суффиксы `L`/`f`, деление `int`/`double`, переполнение | Базовые преобразования `int`/`double`, арифметика, методы | Среднее значение; сумма без переполнения; `byte`/`short`/`float`; отсечение дробной части |
| 4. Пакеты и структура исходников | `package`, `import`, размещение классов, имя пакета, разделение кода по смыслу | `public class`, `import` из Base | Расчет доставки; мини-структура товара и расчета в нескольких пакетах |

**Фактическая практика модуля:** 12 задач от диагностики Base до выбора типов, numeric promotion, защиты от переполнения и разнесения кода по пакетам.

**Контроль модуля:** отдельный список вопросов для устного ревью. Задача `1.0.12` выполняет роль интеграционной работы по пакетам; отдельный мини-проект и README в текущей реализации модуля не предусмотрены.

### Модуль 2. Классы по-взрослому: инкапсуляция, конструкторы, состояние

| Лекция | Чему учит | Обязательные темы до лекции | Практические задачи |
|---|---|---|---|
| 1. Инкапсуляция и инвариант | `private`, управляемые операции, getters/setters без автоматизма, проверка до изменения | Простые классы Base, поля и методы | Переделать `BankAccount` с открытых полей на `private` |
| 2. Конструктор, `this` и валидация | Создание валидного объекта, параметры конструктора, `this`, конструктор по умолчанию | `new`, поля, условия | `Product(name, price, quantity)` с проверками |
| 3. Перегрузка, `static`, `final` и константы | `this(...)`, несколько способов создания, состояние объекта и класса, константы | Конструкторы, Base `static` методы | Добавить сокращенный конструктор; исправить неверный `static quantity` |
| 4. Композиция объектов | Один объект содержит другой, обязательные связи через конструктор | Конструкторы, ссылки | Модель заказа с адресом доставки |
...
| Базовые `int` -> `double` и `(int) double` | Да | Коротко | Base |
| Полная система numeric promotion/cast/overflow | Нет | Да | Все примитивы |
| `package` | Нет | Нет, новая тема | `import`, class |
| `private` | Нет | Нет, новая тема | Base classes |
| Вызов готового конструктора через `new` | Да | Нет | Base |
| Объявление собственных constructors | Нет | Нет, новая тема | `new`, fields |
...
```text
Base: int/double/boolean/char/String, enhanced for,
      int -> double и (int) double
↓
...
- Устное ревью по переходному модулю
- Практика: 12 задач по рефакторингу, типам, преобразованиям, переполнению и пакетам
- Интеграционная задача: мини-структура товара и расчета

### Модуль 2. Инкапсуляция и конструкторы

- Лекция 1. Инкапсуляция и инвариант объекта
- Лекция 2. Конструктор, `this` и валидация
- Лекция 3. Перегрузка, `static`, `final` и константы
- Лекция 4. Композиция объектов
...
- методы, условия, циклы, включая enhanced `for`;
